...
#	endif
	...
	private:
	ProcessType clear_(LengthType index_) DD_NOEXCEPT {
	...
	PointerType allocate(SizeType size_) DD_NOEXCEPT_AS(fetch_(get_index_(size_))) {
		if (size_ <= maximum_size) {
			...
			return BasicAllocatorType::allocate(size_);
		}
	}
	...
	ProcessType deallocate(PointerType pointer_, SizeType size_) DD_NOEXCEPT {
		if (size_ <= maximum_size) {
			...
			BasicAllocatorType::deallocate(pointer_, size_);
		}
	}
...
struct Pool_<AllocatorT_, true> {
	public:
	DD_ALIAS(ThisType, Pool_<AllocatorT_ DD_COMMA true>);
	DD_ALIAS(AllocatorType, AllocatorT_);
	DD_ALIAS(ValueType, void);
...
#	if __cplusplus >= 201103L
	Pool_() = default;
#	else
	Pool_() throw() : m_free_lists_{{}}, m_allocator_() {
	}
#	endif
	...
	private:
	PointerType fetch_(LengthType index_) DD_NOEXCEPT_AS(
		static_cast<PointerType>(m_allocator_.allocate(get_size_(index_)))
	) {
		PointerType temp_;
		if (m_free_lists_[index_]) {
			temp_ = m_free_lists_[index_];
			...
			temp_ = m_allocator_.allocate(get_size_(index_));
		}
		return temp_;
	}
	...
	private:
	ProcessType recycle_(PointerType pointer_, LengthType index_) DD_NOEXCEPT {
		get_next_(pointer_) = m_free_lists_[index_];
		m_free_lists_[index_] = pointer_;
	}
	...
	ProcessType breed_(LengthType index_) DD_NOEXCEPT_AS(
		...
		PointerType temp_ = m_allocator_.allocate(get_size_(index_));
		recycle_(temp_, index_);
	}
	...
	ProcessType clear_(LengthType index_) DD_NOEXCEPT {
		for (PointerType temp_ = m_free_lists_[index_]; temp_; m_free_lists_[index_] = temp_) {
			temp_ = get_next_(temp_);
			m_allocator_.deallocate(m_free_lists_[index_], get_size_(index_));
		}
	}
	...
	PointerType allocate(SizeType size_) DD_NOEXCEPT_AS(fetch_(get_index_(size_))) {
		if (size_ <= maximum_size) {
			...
			return m_allocator_.allocate(size_);
		}
	}
	...
	public:
	ProcessType deallocate(PointerType pointer_) DD_NOEXCEPT {
		m_allocator_.deallocate(pointer_);
	}
	...
	ProcessType deallocate(PointerType pointer_, SizeType size_) DD_NOEXCEPT {
		if (size_ <= maximum_size) {
			...
			m_allocator_.deallocate(pointer_, size_);
		}
	}
