...
struct _IsSorted {
	...
	static ValidityType _is_sorted(
		...
		if (__begin_ != __end_) {
			_UndirectionalIteratorT_ __temp_(__begin_);
			for (; ++__temp_ != __end_; __begin_ = __temp_) {
				if (*__temp_ < *__begin_) {
					return false;
				}
			}
...
struct _IsSorted<true> {
	...
	static ValidityType _is_sorted(
		...
		_FreeAccessIteratorT_ const& __end_,
		_BinaryFunctionT_ const& __less_
	) DD_NOEXCEPT_AS(++__begin_ < __end_ && __less_(*__begin_, *(__begin_ - 1))) {
		while (++__begin_ < __end_) {
